import React, {useEffect, useState} from 'react';
import TextField from "../components/textField";
import TextAreaField from "../components/textAreaField";
import MultiSelectField from "../components/multiSelectField";
import {useTags} from "../hooks/useTags";
import articlesAPI from "../api/articles"
import userAPI from "../api/users"
import {useHistory} from "react-router";
import {useUser} from "../hooks/useUser";
import {useArticle} from "../hooks/useArticles";

const NewArticle = ({userNickname, type, articleId=null, userId=null}) => {

    const initialState = {
        title: "",
        short: "",
        content: "",
        tags: []
    }

    const history = useHistory()
    const [loading, setLoading] = useState(type === "new" ? false : true)
    const [data, setData] = useState(initialState)

    const {tags} = useTags()
    const {addArticle, getUserByNickname} = useUser()
    const {getArticleById, updateArticle, addNewArticle} = useArticle()
    // const {addArticle} = articlesAPI
    // const {addNewArticle} = userAPI


    const handleChange = (data) => {
        console.log(data, ' data from HC')
        setData(prevState => ({...prevState, [data.name]: data.value}))
    }

    useEffect(async () => {
        if (type === 'edit') {
            const data = await getArticleById(articleId)
            setData({
                ...data,
                title: data.title,
                short: data.short,
                content: data.content.join("\n\n"),
                tags: data.tags
            })
            setLoading(prevState => !prevState)
        } else {
            setData(initialState)
        }
    }, [type])

    const getTags = () => {
        if (type === "edit") {
            const tagsIndex = []
            tags.forEach((tag, index) => {
                if (data.tags.find(dataTag => tag.value._id === dataTag)) {
                    console.log('зашло')
                    tagsIndex.push(index)
                }
            })
            console.log(tagsIndex)
            return tagsIndex
        }
        console.log('сработала хуйня')
        return null
    }

    const handleCancelClick = () => {
        history.push(`/${userNickname}`)
    }

    const handleSubmit = async (e) => {
        e.preventDefault()
        e.stopPropagation()
        const correctContentSplit = data.content.split("\n\n")
        const correctContent = correctContentSplit.filter(par => par.length !== 0)

        let correctTags = data.tags
        if (typeof data.tags[0] !== "string") {
            correctTags = data.tags.map(tag => tag.value._id)
        }

        if (type === "edit") {
            console.log(data._id, ' tags suka')
            // const result = await updateArticle(data._id, {...data, tags: data.tags.map(tag => tag.value._id), content: correctContent})
            const result = await updateArticle(data._id, {...data, tags: correctTags, content: correctContent})
            // history.push(`/${userNickname}/article/${result._id}`)
            const win = window.open(`/${userNickname}/article/${result._id}`, "_self");
            win.focus();
        }
        if (type === "new") {
            const result = await addNewArticle({...data, tags: data.tags.map(tag => tag.value._id), content: correctContent}, userNickname)
            const ndResult = await addArticle(userId, result._id)
            // history.push(`/${userNickname}/article/${result._id}`)
            const win = window.open(`/${userNickname}/article/${result._id}`, '_self');
            win.focus();
        }
    }

    return (
        <div className="container">
            <div className="row">
                <div className="col-sm-3 mt-4">
                    <h2 className="text-start">
                        {
                            type === "edit"
                            ? "Редактирование статьи"
                            : "Новая статья"
                        }
                    </h2>
                </div>
            </div>
            <div className="row">
                {
                    !loading
                    ? <form onSubmit={handleSubmit} className="w-100 mt-4 d-flex flex-column align-items-center">
                        <TextField
                            value={data.title}
                            onChange={handleChange}
                            name="title"
                            type="text"
                            label="Название статьи"
                            styles={{}}
                            className="col-sm-6 mb-3"
                        />
                        <TextAreaField
                            value={data.short}
                            name="short"
                            type="text"
                            onChange={handleChange}
                            label="Короткое описание"
                            rows={3}
                            styles={{}}
                            className="col-sm-6 mb-3"
                        />
                        <TextAreaField
                            value={data.content}
                            name="content"
                            type="text"
                            onChange={handleChange}
                            label="Статья"
                            rows={10}
                            styles={{}}
                            className="col-sm-8 mb-4"
                        />
                        <span>Выбрать тэги (максимум 2)</span>
                        <MultiSelectField
                            name="tags"
                            options={tags}
                            className={`basic-multi-select mt-2 text-dark ms-3 col-sm-6`}
                            onChange={handleChange}
                            limit={2}
                            // value={data.tags}
                            value={getTags()?.map(tag => tags[tag])}
                        />
                        <div className="buttonsGroup mt-5">
                            <button type="submit" className="btn btn-primary me-3">Сохранить</button>
                            <button onClick={handleCancelClick} className="btn btn-secondary">Отмена</button>
                        </div>
                    </form>
                    : <p>loading</p>
                }
            </div>
        </div>
    );
};

export default NewArticle;
